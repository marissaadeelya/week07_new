...
```
## This Explains Correlation & Introduce Group of Drugs Together
## First steps

Let's load the prescribing data from July 2025 (same file as in the last worksheet). We'll use the `clean_names()` function from the janitor library to have uniform names in all files we upload today.
...
We'll load in a file with data from the 2022 Scottish Census, telling us the number of people who selected each of the response options in response to a question about their general health. To get this dataset, you need to go to the Scotland's Census website and search by topic: <https://www.scotlandscensus.gov.uk/>. Click on the "Search Data" button on the right hand side. Make sure you have 2022 selected. Click on "Search data by topic". Choose "Health, disability and unpaid care" and pick the first result - "General health" and then "Data Zone 2011" and then "Select All". Select comma separated values in the top right corner. Then click "Download Table". The resulting page will give you the option to download a dataset. Save it in your `data` subfolder as "UV302_general_health_census.csv", and we'll load it into R using the code below:

```{r}
general_health <- read_csv(here("data", "UV302_general_health_census.csv"), skip = 10)  %>%
  clean_names() %>% 
...
You should now have four data sets: `data_july2025`, `gp_addresses`, `data_zones` and `general_health`. Join them together.

```{r}
joined_data <- data_july2025 %>% 
...
#combining by = "data zone", because after combined for the 2nd line, it has new column of "data zone"
```
...
Adapt the code from that you did when you filtered for PARACETAMOL in a previous lab to find out the number of paracetamol tablets/capsules prescribed per person in each data zone in July 2025. Create a plot to look at the distribution of this variable.
...
#A Spearman’s rank correlation showed a weak but statistically significant positive association between the proportion of residents reporting bad health and paracetamol prescribing per head across data zones (ρ = 0.31, p < 0.001).
```
...
Response: The proportion of bad health is per data zone, and the number of prescriptions is per gp practice. We know that not every data zone has a GP practice, so this is a limitation of the data we are dealing with.
...
```{r}
age_data <- read_csv(here("data", "UV110_age_groups_census.csv"), skip = 10) %>% 
  filter(row_number() != 1) %>% # remove the first row (with extraneous information)
  rename(DataZone = `Adult lifestage - 22 groups, all`,
...
parkinsons_data <- data_july2025 %>% 
  filter(str_detect(bnf_item_description, paste(parkinsons_drugs, collapse = "|")))

#That’s a regular expression meaning:Match “Levodopa” OR “Selegiline” OR “Ropinirole”."str" only detect one, so collapse make 3 diff groups into 1
```
...
# the function drop_na() means:Remove all rows in the dataset that contain any missing values (NA) in any column.

cor.test(
  parkinsons_drugs$no_over_65,
  parkinsons_drugs$drugs_paid,
  method = "spearman",
  exact = FALSE
)
