...
We'll focus on the July 2025 file, as that includes recent data. You can find the file and its specific data dictionary here:
...
#filter(!is.na(bnf_item_description)) removes any rows where the bnf_item_description is missing (NA).
#The ! symbol means “not”, so !is.na(...) reverses the result — it becomes TRUE for rows that are not missing.
#arrange(-quantity_sum) → the minus sign (-) means descending order, so it sorts from largest to smallest.
#summarise (quantity_sum): name of the new column
#gt: put into table (as diagram instead)
```
...
HINT: Don't just download the URL above, you need to go to the website and find a link to a .csv file.
...
#It matches rows where:the column hbt in data_july 2025 equals the column hb in HB_lookup.
#After joining, you’re keeping only a few columns:hb_name — the full name of the NHS board (added from HB_lookup),and all columns from hbt to paid_date_month (everything between those in your dataframe).
```
...
```{r}
missing_mystery <- data_july2025 %>% 
  filter(is.na(hb_name))

#is.na = value missing is TRUE
#filter() keeps only the rows where the condition inside is TRUE.So this keeps only rows where hb_name is missing (NA).
#missing_mystery will contain all rows from data_july2025 where the NHS board name couldn’t be matched during your earlier full_join()
```

According to the data dictionary, an hbt code of SB0806 indicates that this medication was prescribed by the Ambulance Service, not a GP practice.(refer to nhs website lnk above)
...
#if 'rename' : new name first
#mutate() → creates or modifies a column.paste("NHS", hb_name) → joins the word "NHS" with the existing value of hb_name, separated by a space.
...
```{r}
paracetamol_data <- data_july2025 %>% 
...
#After summing up the total paid quantity per drug and region, you want to see only the top 3 most prescribed Paracetamol items per NHS board — not all of them. That’s what slice_max() does.
#str_detect() lets you filter all medicines that contain a keyword
```
...
paracetamol_per_head <- data_july2025 %>% 
...
##filter(!is.na(bnf_item_description)) removes any rows where the bnf_item_description is missing (NA)
# geom_bar(stat = "identity") → draw bars using the actual numeric values in your data (e.g. prescription quantity), instead of automatically counting rows.
```
